...
const CITY=window.TCESP_CITY,OWNER="transparencia-sp",APP_VERSION="v1.1.0-central";
...
function auditAnalyzablePayments(rows){
  return rows.filter(x=>x.evento==="Valor Pago"&&n(x.valor)>0&&!auditExcludedSupplier(x));
}
function supplierConcentration(rows){
  const payments=auditAnalyzablePayments(rows),groups=supplierGroups(payments).map(g=>({key:g.id,name:g.nome,id:g.id,paid:n(g.e["Valor Pago"])})).filter(g=>g.paid>0).sort((a,b)=>b.paid-a.paid),total=groups.reduce((s,g)=>s+g.paid,0);
  const share=k=>total?groups.slice(0,k).reduce((s,g)=>s+g.paid,0)/total*100:0;
  const hhi=total?groups.reduce((s,g)=>{let q=g.paid/total;return s+q*q*10000},0):0;
  return {payments,groups,total,top1:share(1),top5:share(5),top10:share(10),hhi};
}
function nearLimitSignals(rows){
  return commitmentAuditRecords(rows).filter(z=>z.net>0&&z.limit>0&&!auditExcludedSupplier(z.sample)&&z.net/z.limit>=.95&&z.net<=z.limit).map(z=>({...z,ratio:z.net/z.limit})).sort((a,b)=>b.ratio-a.ratio||b.net-a.net);
}
function physicalPersonSignals(rows){
  const isPF=x=>/CPF|PESSOA F[ÍI]SICA/i.test(String(x.fornecedorId||""));
  const pay=rows.filter(x=>x.evento==="Valor Pago"&&n(x.valor)>0&&isPF(x));
  const map=new Map();
  pay.forEach(x=>{let k=supplierKey(x),z=map.get(k);if(!z){z={key:k,name:x.fornecedor||"(sem nome)",id:x.fornecedorId||"",paid:0,count:0,emps:new Set()};map.set(k,z)}z.paid+=n(x.valor);z.count++;z.emps.add(x.empenho||"—")});
  return [...map.values()].map(z=>({...z,emps:[...z.emps]})).sort((a,b)=>b.paid-a.paid||b.count-a.count);
}
function auditOrgBreakdown(y){
  const kinds=[{label:"Prefeitura",test:o=>orgMatch(o,"Prefeitura")},{label:"Câmara",test:o=>orgMatch(o,"Câmara")}];
  return kinds.map(k=>{let rows=db.despesas.filter(x=>+x.ano===y&&k.test(x.orgao)),e=evSums(rows),q=qualitySignals(rows,[]),pf=physicalPersonSignals(rows);return {label:k.label,rows,paid:n(e["Valor Pago"]),net:net(e),pfTotal:pf.reduce((s,z)=>s+z.paid,0),pfPeople:pf.length,weekend:q.weekend.length}});
}
function evidenceMatrixRows(){
  return [
    ["Empenhos pequenos / perto do limite","Processo de contratação, ata/contrato e identificação do objeto","Verificar se os empenhos pertencem ao mesmo objeto e qual procedimento os ampara"],
    ["Pagamentos coincidentes","Nota fiscal, medição, liquidação e fonte de recurso","Distinguir duplicidade de divisão legítima por fonte/etapa"],
    ["Pessoa física fora da folha","Portaria, lei de diárias, relatório de viagem/adiantamento e prestação de contas","Confirmar a natureza jurídica do pagamento"],
    ["Concentração em credores","Editais, atas de registro de preços, contratos e pesquisa de mercado","Distinguir concentração decorrente de mercado/contrato de dependência atípica"],
    ["Preço unitário / quantidade","Planilha de itens, quantidades, unidades e preços","A base TCESP usada aqui não traz granularidade suficiente para teste de sobrepreço"],
    ["Origem do recurso → despesa","Empenho com fonte/destinação, conta vinculada e extrato","A base de receitas e despesas aqui não permite vinculação segura entre uma entrada específica e um pagamento"]
  ];
}
function audit(){
  const {y,d,r,org}=auditYearRows(),coverage=[...new Set(d.map(x=>+x.mesNum).filter(Boolean))].sort((a,b)=>a-b),last=coverage.length?Math.max(...coverage):0,partial=coverage.length<12,frag=fragmentationSignals(d),coinc=coincidentPaymentSignals(d),q=qualitySignals(d,r),integ=stageIntegrity(d),conc=supplierConcentration(d),near=nearLimitSignals(d),pf=physicalPersonSignals(d),orgs=auditOrgBreakdown(y);
  ...
  const nearRows=near.slice(0,30).map((z,i)=>`<tr><td>${i+1}</td><td><button class="audit-link" type="button" data-dossier-key="${esc(z.key)}">${esc(z.name)}</button></td><td>${esc(z.emp)}</td><td class="num">${brl(z.net)}</td><td class="num">${brl(z.limit)}</td><td class="num">${(z.ratio*100).toLocaleString("pt-BR",{maximumFractionDigits:1})}%</td></tr>`).join("");
  const pfRows=pf.slice(0,30).map((z,i)=>`<tr><td>${i+1}</td><td><button class="audit-link" type="button" data-dossier-key="${esc(z.key)}">${esc(z.name)}</button><small>${esc(displaySupplierId(z.id||z.key))}</small></td><td class="num">${brl(z.paid)}</td><td class="num">${z.count}</td><td>${z.emps.slice(0,8).map(esc).join(", ")}${z.emps.length>8?` +${z.emps.length-8}`:""}</td></tr>`).join("");
  const concRows=conc.groups.slice(0,15).map((z,i)=>`<tr><td>${i+1}</td><td><button class="audit-link" type="button" data-dossier-key="${esc(z.key)}">${esc(z.name)}</button></td><td class="num">${brl(z.paid)}</td><td class="num">${conc.total?(z.paid/conc.total*100).toLocaleString("pt-BR",{maximumFractionDigits:2}):"0"}%</td></tr>`).join("");
  const orgRows=orgs.map(z=>`<tr><td><strong>${z.label}</strong></td><td class="num">${brl(z.net)}</td><td class="num">${brl(z.paid)}</td><td class="num">${brl(z.pfTotal)}</td><td class="num">${z.pfPeople}</td><td class="num">${z.weekend}</td></tr>`).join("");
  const evidenceRows=evidenceMatrixRows().map(z=>`<tr><td><strong>${z[0]}</strong></td><td>${z[1]}</td><td>${z[2]}</td></tr>`).join("");
  ...
  $("#auditSec").innerHTML=`<section class="audit-hero card"><div><small>Sinais de auditoria • ${APP_VERSION}</small><h2>Triagem automática do exercício ${y}</h2><p>Estes testes apontam padrões que merecem cotejo documental; <strong>não qualificam a despesa como irregular</strong>. Para reduzir falsos positivos, esta aba usa o exercício inteiro carregado e respeita apenas o filtro de órgão. Mês e pesquisa textual são ignorados.</p></div><span class="audit-scope">${esc(org)} • ${last?`até ${MONTHS[last]}/${y}`:"sem meses carregados"}${partial?" • exercício parcial":""}</span></section>
  <div class="auditgrid audit-quality">${qualityCards.map((z,i)=>`<div class="audititem card ${i===7&&!integrityBad?'ok':'neutral'}"><small>${z[0]}</small><strong>${typeof z[1]==="number"?z[1].toLocaleString("pt-BR"):z[1]}</strong><small>${z[2]}</small></div>`).join("")}</div>
  <section class="audit-section card"><div class="audit-section-head"><div><h3>Concentração dos pagamentos por credor</h3><p>Calculada sobre pagamentos positivos a credores analisáveis, excluindo folha, Município/Câmara, encargos, bancos, energia/telefonia e cartões de benefício. Mede concentração, não irregularidade.</p></div><strong>${brl(conc.total)} analisado</strong></div><div class="auditgrid audit-quality"><div class="audititem card neutral"><small>Maior credor</small><strong>${conc.top1.toLocaleString("pt-BR",{maximumFractionDigits:1})}%</strong><small>Participação no total analisado</small></div><div class="audititem card neutral"><small>Top 5</small><strong>${conc.top5.toLocaleString("pt-BR",{maximumFractionDigits:1})}%</strong><small>Participação acumulada</small></div><div class="audititem card neutral"><small>Top 10</small><strong>${conc.top10.toLocaleString("pt-BR",{maximumFractionDigits:1})}%</strong><small>Participação acumulada</small></div><div class="audititem card neutral"><small>HHI</small><strong>${conc.hhi.toLocaleString("pt-BR",{maximumFractionDigits:0})}</strong><small>Escala 0–10.000; maior = pagamentos mais concentrados</small></div></div><div class="audit-tablewrap"><table class="audit-table"><thead><tr><th>#</th><th>Credor</th><th>Pago</th><th>% do total analisado</th></tr></thead><tbody>${concRows||'<tr><td colspan="4">Sem pagamentos analisáveis.</td></tr>'}</tbody></table></div></section>
  <section class="audit-section card"><div class="audit-section-head"><div><h3>Prefeitura × Câmara</h3><p>Quadro separado para evitar que padrões da Câmara desapareçam dentro do total municipal.</p></div></div><div class="audit-tablewrap"><table class="audit-table"><thead><tr><th>Órgão</th><th>Empenho líquido</th><th>Pago</th><th>Pago a pessoas físicas</th><th>Pessoas físicas</th><th>Pagamentos em fim de semana</th></tr></thead><tbody>${orgRows}</tbody></table></div></section>
  <section class="audit-section card"><div class="audit-section-head"><div><h3>Pessoas físicas fora da folha — triagem</h3><p>Pagamentos positivos cujo identificador da base indica CPF/pessoa física. A natureza não é inferida: pode ser diária, ressarcimento, adiantamento, indenização, aquisição ou outro pagamento legítimo.</p></div><strong>${pf.length.toLocaleString("pt-BR")} pessoa(s)</strong></div><div class="audit-tablewrap"><table class="audit-table"><thead><tr><th>#</th><th>Pessoa</th><th>Total pago</th><th>Lançamentos</th><th>Empenhos</th></tr></thead><tbody>${pfRows||'<tr><td colspan="5">Nenhum pagamento selecionado.</td></tr>'}</tbody></table></div></section>
  <section class="audit-section card"><div class="audit-section-head"><div><h3>Empenhos entre 95% e 100% do limite de referência</h3><p>Teste de proximidade aritmética. Estar perto do limite não demonstra dispensa nem fracionamento; é necessário identificar objeto, procedimento e eventual ata/contrato.</p></div><strong>${near.length.toLocaleString("pt-BR")} empenho(s)</strong></div><div class="audit-tablewrap"><table class="audit-table"><thead><tr><th>#</th><th>Credor</th><th>Empenho</th><th>Valor</th><th>Limite de referência</th><th>% do limite</th></tr></thead><tbody>${nearRows||'<tr><td colspan="6">Nenhum empenho selecionado.</td></tr>'}</tbody></table></div></section>
  <section class="audit-section card"><div class="audit-section-head"><div><h3>Múltiplos empenhos de pequeno valor por credor</h3><p>Triagem de concentração anual de empenhos individualmente pequenos. O padrão pode decorrer de licitação, ata de registro de preços, parcelamento legítimo ou outras causas; a confirmação depende do processo administrativo. A regra detalhada de seleção não é exibida.</p></div><strong>${frag.length.toLocaleString("pt-BR")} credor(es)</strong></div><div class="audit-tablewrap"><table class="audit-table"><thead><tr><th>#</th><th>Credor</th><th>Empenhos selecionados</th><th>Soma</th><th>Maior empenho</th><th>Nº dos empenhos</th></tr></thead><tbody>${fragRows||'<tr><td colspan="6">Nenhum padrão selecionado neste exercício.</td></tr>'}</tbody></table></div></section>
  <section class="audit-section card"><div class="audit-section-head"><div><h3>Pagamentos coincidentes</h3><p>Mesmo credor, mesmo valor e mesma data em empenhos distintos. Há hipóteses legítimas, inclusive divisão por fontes de recursos; por isso a tabela serve para cotejo com notas fiscais, medições e liquidações.</p></div><strong>${coinc.length.toLocaleString("pt-BR")} grupo(s)</strong></div><div class="audit-tablewrap"><table class="audit-table"><thead><tr><th>#</th><th>Data</th><th>Credor</th><th>Valor unitário</th><th>Ocorrências</th><th>Empenhos</th><th>Valor coincidente além da 1ª ocorrência</th></tr></thead><tbody>${coincRows||'<tr><td colspan="7">Nenhuma coincidência selecionada neste exercício.</td></tr>'}</tbody></table></div></section>
  <section class="audit-section card"><h3>Matriz de evidências para confirmação</h3><p>O painel separa o que pode ser calculado dos lançamentos daquilo que exige documento externo. Isso reduz conclusões indevidas a partir de um sinal estatístico.</p><div class="audit-tablewrap"><table class="audit-table"><thead><tr><th>Sinal</th><th>Documento necessário</th><th>Objetivo da conferência</th></tr></thead><tbody>${evidenceRows}</tbody></table></div></section>
  <section class="audit-section card"><h3>Limitações: preço unitário e rastreamento do recurso</h3><p>Com os campos disponíveis nesta base não é possível testar de forma robusta preço unitário, quantidade, sobrepreço ou vincular uma receita específica a um pagamento específico. Para isso são necessários itens/quantidades dos processos de contratação e a fonte/destinação do empenho, além de extratos de conta vinculada quando houver.</p></section>`;
